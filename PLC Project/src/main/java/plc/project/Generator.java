...
public final class Generator implements Ast.Visitor<Void> {
    ...
    public Void visit(Ast.Method ast) {
        /*Generates a method expression. The method should begin with the method's JVM type name
        followed by the method name, both of which are found in the AST. Then the method should
        ...
        Each parameter will consist of a JVM type name and the parameter name.
         ...
         If the statements is empty the closing brace should also be on the same line, otherwise
         each statement is generated on a new line with increased indentation followed by a closing
         brace on a new line with the original indentation.
         */
        ...
        if(!ast.getParameters().isEmpty()) {
            for (int i = 0; i < ast.getParameters().size(); i++) {
                //loops to print all parameters
                print(ast.getParameterTypeNames().get(i), " ", ast.getParameters().get(i));
                if (i != ast.getParameters().size()) {
                    //only adds comma if not at the end of the list
                    print (", ");
                }
            }
        }
